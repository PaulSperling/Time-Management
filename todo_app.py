...
from PyQt5.QtWidgets import (
    ...
    QCalendarWidget, QFrame, QSizePolicy, QMenu, QScrollArea,
)
from PyQt5.QtCore import (
    Qt, QDate, QTimer, QAbstractTableModel, QModelIndex,
    QSortFilterProxyModel, pyqtSignal, QRectF, QUrl, QEvent,
)
...
class Task:
    def __init__(self, name, description, due_date, do_date, priority,
                 time_estimate, done=False, project="", link=""):
        self.name = name
...
def is_overdue(task):
    ...
    if task.due_date < today:
        return True
    if task.due_date == today and datetime.now().hour >= 17:
        return True
    return False
...
def _is_url(text):
    return text.lower().startswith(('http://', 'https://', 'ftp://'))
...
class TaskDialog(QDialog):
    ...
    def _build(self):
        ...
        self.link_edit.setPlaceholderText("https://… or C:\\path\\to\\file")

        layout.addRow("Name *",        self.name_edit)
        layout.addRow("Project",       self.project_combo)
        layout.addRow("Description",   self.desc_edit)
        layout.addRow("External Link", self.link_edit)
...
COLUMNS = ['Name', 'Project', 'Due Date', 'Do Date', 'Priority',
           'Est. Time', 'Urgency', 'Do Today', 'Done', 'Link']
...
class TaskTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            if c == COL['Link']:      return "↗" if t.link else "—"
        ...
        if role == Qt.UserRole:
            ...
            if c == COL['Link']:      return int(bool(t.link))
        ...
        if role == Qt.ForegroundRole:
            ...
            if c == COL['Link']:
                return QColor(137, 180, 250) if t.link else TEXT_MUTED
            return TEXT_PRIMARY
...
class TaskPage(QWidget):
    ...
    def _build(self):
        ...
        self.link_label.setStyleSheet(
            "background: #1e1e30; color: #89b4fa; font-size: 11px; "
            "padding: 5px 10px; border: 1px solid #2a2a42; border-radius: 5px;"
        )
        self.link_label.setFixedHeight(30)
        self.link_label.setAlignment(Qt.AlignVCenter | Qt.AlignLeft)
        self.link_label.setText('<span style="color: #45475a; font-style: italic;">no link</span>')
    ...
    def _filtered_tasks(self):
        ...
        for t in self.tasks:
            ...
            if f == "Overdue" and not is_overdue(t): continue
        ...
        if src.isValid() and 0 <= src.row() < len(self.model.tasks):
            ...
            if t.link:
                ...
                self.link_label.setText(
                    f'<a href="{href}" style="color: #89b4fa; text-decoration: none;">↗ {t.link}</a>'
                )
            else:
                self.link_label.setText('<span style="color: #45475a; font-style: italic;">no link</span>')
    ...
    def _on_cell_clicked(self, proxy_index):
        ...
        if 0 <= src.row() < len(self.model.tasks):
            task = self.model.tasks[src.row()]
            if task.link:
...
class OverviewPage(QWidget):
    ...
    def _build(self):
        ...
        self._eod_btn.setEnabled(False)
        self._eod_btn.setToolTip("Available at 5:00 PM")
        self._eod_btn.clicked.connect(self._show_eod_summary)
        inner.addWidget(self._eod_btn)
    ...
    def _update_time(self):
        ...
        self._eod_btn.setEnabled(past_eod)
        self._eod_btn.setToolTip("" if past_eod else "Available at 5:00 PM")
    ...
    def _show_eod_summary(self):
        dlg = EndOfDaySummaryDialog(self, self.tasks, self.projects)
        dlg.exec_()
        if dlg.rescheduled:
            self.tasks_changed.emit()
        self.refresh()
...
class PieChartWidget(QWidget):
    ...
    def update_data(self, tasks, projects):
        ...
        for t in tasks:
            if not t.done:
                ...
                hours[key] = hours.get(key, 0.0) + t.time_estimate

        self.segments = []
        for name, total_h in sorted(hours.items(), key=lambda x: x[1], reverse=True):
            if name == "(No Project)":
                ...
                color = QColor(88, 91, 112)
            self.segments.append((name, total_h, color))
        self.update()
    ...
    def paintEvent(self, event):
        ...
        total = sum(s[1] for s in self.segments)  # total hours
        ...
        for name, hours, color in self.segments:
            span = int(round(360 * hours / total * 16))
        ...
        for i, (name, hours, color) in enumerate(self.segments):
            y = ly + i * row_h
            # swatch
            ...
            painter.drawText(
                ...
                f"{display}  {hours:.1f}h ({pct:.0f}%)"
            )



class WorkloadBarChartWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.day_data = []  # [(day_abbr, day_num_str, pct), ...]
    ...
    def update_data(self, tasks):
        ...
        for wd in working_days:
            total_h = sum(
                t.time_estimate for t in tasks
                if not t.done and t.do_date == wd
            )
            pct = (total_h / 8.0) * 100.0
            self.day_data.append((wd.strftime("%a"), str(wd.day), pct))
        self.update()
    ...
    def paintEvent(self, event):
        ...
        if chart_h <= 0 or chart_w <= 0 or not self.day_data:
            return

        # Grid lines at 0 / 50 / 100 / 150 %
        painter.setFont(QFont("Segoe UI", 7))
        for mark in (0, 50, 100, 150):
            y = margin_top + chart_h * (1.0 - mark / MAX_PCT)
            if mark == 100:
                pen = QPen(QColor(205, 214, 244, 90), 1, Qt.DashLine)
            else:
                pen = QPen(QColor(88, 91, 112, 55), 1, Qt.DashLine)
            painter.setPen(pen)
            painter.drawLine(int(margin_left), int(y), int(margin_left + chart_w), int(y))
            painter.setPen(TEXT_MUTED)
            painter.drawText(
                QRectF(0, y - 8, margin_left - 3, 16),
        ...
        for i, (day_name, day_num, pct) in enumerate(self.day_data):
            bx = margin_left + bar_gap + i * (bar_w + bar_gap)
            capped = min(pct, MAX_PCT)
            bh = chart_h * capped / MAX_PCT
            by = margin_top + chart_h - bh

            if pct < 60:
                color = QColor(137, 180, 250)   # blue
            elif pct <= 100:
                color = QColor(166, 227, 161)   # green
            elif pct <= 125:
                color = QColor(249, 226, 175)   # yellow
            else:
                color = QColor(243, 139, 168)   # red

            painter.setPen(Qt.NoPen)
            painter.setBrush(color)
            painter.drawRoundedRect(QRectF(bx, by, bar_w, bh), 2, 2)

            painter.setPen(TEXT_MUTED)
            painter.setFont(QFont("Segoe UI", 7))
            painter.drawText(
                QRectF(bx, margin_top + chart_h + 3, bar_w, 12),
                Qt.AlignHCenter | Qt.AlignTop,
                day_name
            )
            painter.drawText(
                QRectF(bx, margin_top + chart_h + 15, bar_w, 12),
                Qt.AlignHCenter | Qt.AlignTop,
                day_num
            )
...
class ProjectPanel(QWidget):
    ...
    def _build(self):
        ...
        # workload outlook bar chart
        wk_lbl = QLabel("WORKLOAD OUTLOOK")
        wk_lbl.setStyleSheet("color: #585b70; font-size: 9px; font-weight: bold; letter-spacing: 1px;")
        inner.addWidget(wk_lbl)

        self.workload = WorkloadBarChartWidget()
        inner.addWidget(self.workload, stretch=1)
        ...
        req_lbl = QLabel("FEATURE REQUESTS")
        req_lbl.setStyleSheet("color: #585b70; font-size: 9px; font-weight: bold; letter-spacing: 1px;")
        inner.addWidget(req_lbl)

        self.request_edit = QTextEdit()
        self.request_edit.setPlaceholderText("Describe a feature or improvement…")
        self.request_edit.setMaximumHeight(70)
        inner.addWidget(self.request_edit)
    ...
    def _submit_request(self):
        ...
        filepath = os.path.join(save_dir, "feature_requests.txt")
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
        with open(filepath, "a", encoding="utf-8") as f:
            f.write(f"[{timestamp}]\n{text}\n\n")
        self.request_edit.clear()
        QMessageBox.information(self, "Submitted", "Feature request saved.")

    def refresh_pie(self):
        self.pie.update_data(self.tasks, self.projects)
        self.workload.update_data(self.tasks)
...
class EndOfDaySummaryDialog(QDialog):
    def __init__(self, parent, tasks, projects):
        super().__init__(parent)
        self.tasks = tasks
        self.projects = projects
        self.rescheduled = False
        self._planned_rows = []  # [(task, do_edit)]
        self._overdue_rows = []  # [(task, do_edit, due_edit)]
        self.setWindowTitle(f"End of Day Summary — {date.today().strftime('%B %d, %Y')}")
    ...
    def _build(self):
        ...
        # — Completed today —
        completed = [t for t in self.tasks if t.done and t.do_date == today]
        layout.addWidget(self._sec("COMPLETED TODAY"))
        if completed:
            total_h = sum(t.time_estimate for t in completed)
            for t in completed:
                row = QHBoxLayout()
                nl = QLabel(f"  ✓  {t.name}")
                nl.setStyleSheet("color: #a6e3a1; font-weight: bold;")
                pl = QLabel(t.project or "—")
                pl.setStyleSheet("color: #585b70; font-size: 11px;")
                tl = QLabel(f"{t.time_estimate:.1f}h")
                tl.setStyleSheet("color: #585b70; font-size: 11px;")
                row.addWidget(nl); row.addStretch()
                row.addWidget(pl); row.addSpacing(12); row.addWidget(tl)
                layout.addLayout(row)
            tot = QLabel(f"  Total: {total_h:.1f}h across {len(completed)} task(s)")
            tot.setStyleSheet("color: #585b70; font-size: 11px; padding-left: 4px;")
            layout.addWidget(tot)
        else:
            layout.addWidget(self._none("No tasks completed today."))

        layout.addWidget(h_rule())

        # — Planned but not done (exclude overdue — they appear in the next section) —
        planned = [t for t in self.tasks if not t.done and t.do_date == today and not is_overdue(t)]
        layout.addWidget(self._sec("PLANNED BUT NOT DONE"))
        if planned:
            note = QLabel("  Defer the do date for any tasks you didn't get to.")
            note.setStyleSheet("color: #585b70; font-size: 11px; padding-left: 4px;")
            layout.addWidget(note)

            scroll_p = QScrollArea()
            scroll_p.setWidgetResizable(True)
            scroll_p.setMaximumHeight(200)
            cont_p = QWidget()
            cl_p = QVBoxLayout(cont_p)
            cl_p.setSpacing(6)
            cl_p.setContentsMargins(0, 4, 4, 4)

            tomorrow = QDate.currentDate().addDays(1)
            for t in planned:
                card = QFrame()
                card.setStyleSheet(
                    "QFrame { background: #252538; border: 1px solid #2e2e48; border-radius: 6px; }"
                )
                fl = QFormLayout(card)
                ...
                header = QHBoxLayout()
                nl = QLabel(f"○  {t.name}")
                nl.setStyleSheet("color: #f9e2af; font-weight: bold; background: transparent; border: none;")
                pl = QLabel(t.project or "(No Project)")
                pl.setStyleSheet("color: #585b70; font-size: 11px; background: transparent; border: none;")
                header.addWidget(nl); header.addStretch(); header.addWidget(pl)
                fl.addRow(header)

                do_edit = QDateEdit(tomorrow)
                do_edit.setCalendarPopup(True)
                fl.addRow("Defer Do Date to:", do_edit)

                cl_p.addWidget(card)
                self._planned_rows.append((t, do_edit))

            cl_p.addStretch()
            scroll_p.setWidget(cont_p)
            layout.addWidget(scroll_p)
        else:
            layout.addWidget(self._none("All planned tasks were completed!"))

        layout.addWidget(h_rule())

        # — Overdue tasks —
        overdue = [t for t in self.tasks if is_overdue(t)]
        layout.addWidget(self._sec("OVERDUE TASKS"))
        if overdue:
            note = QLabel(f"  {len(overdue)} overdue task(s). Adjust dates and click Save to reschedule.")
            note.setStyleSheet("color: #f38ba8; font-size: 11px; padding-left: 4px;")
            layout.addWidget(note)
            ...
            for t in overdue:
                ...
                header = QHBoxLayout()
                nl = QLabel(t.name)
                nl.setStyleSheet("color: #f38ba8; font-weight: bold; background: transparent; border: none;")
                pl = QLabel(t.project or "(No Project)")
                pl.setStyleSheet("color: #585b70; font-size: 11px; background: transparent; border: none;")
                header.addWidget(nl); header.addStretch(); header.addWidget(pl)
                fl.addRow(header)

                do_edit = QDateEdit(QDate(t.do_date.year, t.do_date.month, t.do_date.day))
                do_edit.setCalendarPopup(True)
                due_edit = QDateEdit(QDate(t.due_date.year, t.due_date.month, t.due_date.day))
                due_edit.setCalendarPopup(True)
                fl.addRow("New Do Date:", do_edit)
                fl.addRow("New Due Date:", due_edit)

                cl.addWidget(card)
                self._overdue_rows.append((t, do_edit, due_edit))

            cl.addStretch()
            scroll.setWidget(container)
            layout.addWidget(scroll)
        else:
            layout.addWidget(self._none("No overdue tasks — great work!"))

        layout.addWidget(h_rule())

        # — Buttons —
        btn_row = QHBoxLayout()
        if overdue or planned:
            save_btn = QPushButton("Save Reschedules && Close")
    ...
    def _none(self, text):
        lbl = QLabel(f"  {text}")
        lbl.setStyleSheet("color: #45475a; font-style: italic; padding-left: 4px;")
        return lbl

    def _save_and_close(self):
        for task, do_edit in self._planned_rows:
            d = do_edit.date()
            task.do_date = date(d.year(), d.month(), d.day())
        for task, do_edit, due_edit in self._overdue_rows:
            d = do_edit.date()
            task.do_date = date(d.year(), d.month(), d.day())
            d2 = due_edit.date()
            task.due_date = date(d2.year(), d2.month(), d2.day())
...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        self._autosave_timer.setInterval(10 * 60 * 1000)  # 10 minutes
    ...
    def closeEvent(self, event):
        ...
        msg.setText(f"<b>{name}</b> has unsaved changes.")
